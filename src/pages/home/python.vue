		...
		<el-main>
			<div>
				<div style="position: fixed;left: 70%; ">
					    <el-steps direction="vertical" :active="activeStep" space="40px" finish-status="wait" process-status="finish">
						<el-step v-for="(item, index) in steps" :key="index" :title="item.title" @click.native="jump(index)" icon="">


						</el-step>
						<el-step icon="el-icon-circle-plus-outline" :key="maxindex" @click.native="dialogFormVisible = true" title="新增">
					...
					<el-dialog title="新增标签" :visible.sync="dialogFormVisible">
						<el-form :model="form">
							...
							<el-form-item label="文本" :label-width="formLabelWidth">
								<el-input v-model="form.text" autocomplete="off" style="width: 90%;" type="textarea" maxlength="300"
								 show-word-limit :autosize="{ minRows: 2, maxRows: 10}"></el-input>
				...
				<div style="margin-left: 1%;">
					<div v-for="(item, index) in steps" :key="index" class="step-jump" style="margin-top: 3% ;">
						...
						<div v-else>
							...
							<br />
							<el-popconfirm placement="bottom-end" confirmButtonText='提交' cancelButtonText='删除' icon="el-icon-info" iconColor="#409EFF"
							 title="提交或者删除" @onConfirm="submit(item,index)" @onCancel="deltitle(item,index)">
	...
	export default {
		...
		methods: {
			...
			submit(item, index) {
				...
				axios.post('http://' + window.location.host.replace(":8079", "") + ":8002/updatapage/", param, {
			...
			deltitle(item, index) {
				...
				axios.post('http://' + window.location.host.replace(":8079", "") + ":8002/del/", param, {
					withCredentials: true
				}).then(res => {
					console.log(res.data);
			...
			addtitle() {
				...
				axios.post('http://' + window.location.host.replace(":8079", "") + ":8002/addpage/", param, {
					withCredentials: true
				}).then(res => {
					let data = res.data
					console.log(data);
			...
			getpagedata() {
				...
				axios.get('http://' + window.location.host.replace(":8079", "") + ":8002/getpage?page=python", {
					withCredentials: true
				}).then(res => {
